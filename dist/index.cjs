...
__export(index_exports, {
  ...
  Trace: () => Trace,
  WebSocketGateWaySymbol: () => WebSocketGateWaySymbol,
...
var startWebSockets = () => {
  const webSocketGateWayList2 = socketMetadata.getGateWayList();
  if (socketMetadata.getGateWayList().length > 0) {
    const server = socketMetadata.getServer();
    server.on("connection", async (socket) => {
      for (let i = 0; i < webSocketGateWayList2.length; i++) {
        const ws = webSocketGateWayList2[i];
        if (ws) {
...
var swaggerUI = async (props) => {
  ...
  const routerHtml = (0, import_express5.Router)().use(
    swaggerUi.serve,
    async (...args) => {
      const doc = await generateSwaggerDoc(props);
